...
- **BREAKING:** Removed the realtime feature. The backend no longer exposes
  MQTT, SSE, WebSocket, or `/auth/iot-credentials`, so the SDK drops
  `client.connectRealtime()`, `RealtimeClient`, the `src/realtime` module
  (topic builders, IoT credential helpers, and their types
  `IotCredentials`, `RealtimeOptions`, `SubscribeFilters`, `TopicParts`,
  `PublishData`), the `iotEndpoint` / `wsUrl` options
  (`AXONPUSH_IOT_ENDPOINT` / `AXONPUSH_WS_URL`), and the `mqtt` dependency.
  Ingest over the REST client, OTLP, or the Sentry DSN compat path. Auth is
  `ak_` API keys (`X-API-Key`) and `pt_` public ingest tokens
  (`X-Public-Token`).
