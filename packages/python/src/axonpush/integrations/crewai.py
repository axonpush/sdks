...
class AxonPushCrewCallbacks:
    ...
    def on_crew_start(self) -> None:
        self._publish(
            "crew.start", EventType.AGENT_START,
            {"framework": "crewai"},
        )
    ...
    def on_step(self, step_output: Any) -> None:
        ...
        if tool:
            ...
            self._publish(
                ...
                {"tool_name": tool_name, "tool_input": str(getattr(step_output, "tool_input", ""))[:500]},
                agent_id=agent_name,
            )
            result = getattr(step_output, "result", None)
            if result is not None:
                self._publish(
                    f"tool.{tool_name}.end", EventType.AGENT_TOOL_CALL_END,
                    {"tool_name": tool_name, "result_preview": str(result)[:500]},
                    agent_id=agent_name,
                )
        else:
            self._publish(
                "agent.step", EventType.AGENT_MESSAGE,
    ...
    def on_task_complete(self, task_output: Any) -> None:
        self._publish(
            "task.complete", EventType.AGENT_END,
            {
