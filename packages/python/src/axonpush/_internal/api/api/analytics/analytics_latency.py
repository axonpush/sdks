...
) -> ErrorModel | LatencyPercentilesDTO:
    if response.status_code == 200:
        response_200 = LatencyPercentilesDTO.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ErrorModel | LatencyPercentilesDTO]:
    return Response(
...
def sync(
    *,
    ...
    operation: str | Unset = UNSET,
    error_type: str | Unset = UNSET,
) -> ErrorModel | LatencyPercentilesDTO | None:
    """Latency + TTFT percentiles (p50/p95/p99/max) over the window
